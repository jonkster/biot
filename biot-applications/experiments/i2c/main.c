...
int imu0_cmd(int argc, char **argv) {
    return setupMPU9150(i2c_dev);
}

int imu1_cmd(int argc, char **argv) {
    imuLoop(i2c_dev);
    return 0;
}

int reinit_cmd(int argc, char **argv) {
    return initI2C();
}
...
static const shell_command_t shell_commands[] = {
    ...
    { "reinit", "re initialise i2c master", reinit_cmd },
    { "imu0", "set up IMU", imu0_cmd },
...
int main(void)
{
    ...
    if (initI2C())
    {
        puts("IMU I2C interface ready!");
    }
    else
    {
        puts("IMU I2C interface NOT ready! :(");
    }
