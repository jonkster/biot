...
void oledClearAll(void)
{
    ...
    for (lineNo = 0; lineNo < SSD1306_PAGECOUNT; lineNo++)
    {
        oledClearLine(lineNo);
    }
    oledSetPageAddress(0);
    oledSetColumnAddress(0);
    oledWriteText("Biotz Node");
}

void oledClearLine(uint8_t lineNo)
{
    oledSetPageAddress(lineNo);
    oledSetColumnAddress(0);
    uint8_t colNumber;
    for (colNumber = 0; colNumber <  SSD1306_COLCOUNT; colNumber++)
    {
        oledWriteData(0x00);
    }
    oledSetColumnAddress(0);
